<template>
    <div class="flex flex-col items-center w-1/3  m-auto h-3/4	">
        ...
        <button class=" border-2 border-purple-700 text-purple-700 p-2 hover:text-white hover:bg-purple-700 mt-4" @click="reserver">Choisir votre creneau</button>
    </div>
</template>
...
export default {
    ...
    setup(props){
        ...
        const change = async ()=>{
            ...
            let res = await fetch("http://brief6.api/rdv/one",{
                method:"POST",
        ...
        const reserver = async ()=>{
            ...
            await fetch("http://brief6.api/rdv/add",{
                    method:"POST",
                    headers: {'Content-Type':'application/json'},
